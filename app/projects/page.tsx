...
// XÜSUSİ SÜNİ İNTELLEKT TƏRCÜMƏ MÜHƏRRİKİ
const AutoTranslate = ({ text, lang }: { text: string, lang: string }) => {
  ...
  useEffect(() => {
    ...
    const translateText = async () => {
      try {
        // Əgər əvvəlki kimi | ilə ayırmısansa, yenə də işləyəcək
        if (text.includes('|')) {
           const parts = text.split('|');
           setTranslated(lang === 'az' ? parts[0].trim() : parts[1].trim());
           return;
        }

        // Google Translate API vasitəsilə avtomatik tərcümə
        const res = await fetch(`https://translate.googleapis.com/translate_a/single?client=gtx&sl=az&tl=${lang}&dt=t&q=${encodeURIComponent(text)}`);
        ...
        setTranslated(text); // İnternet xətası olsa orijinalı göstər
      }
    };
    ...
    <main className="min-h-screen bg-transparent p-4 sm:p-6 md:p-10 pt-24 md:pt-32 transition-colors duration-500 relative z-10">
      <div className="max-w-7xl mx-auto">
        <h1 className="text-3xl md:text-5xl font-black text-slate-900 dark:text-white mb-10 md:mb-16 uppercase tracking-tighter border-l-4 border-emerald-500 pl-4 md:pl-6 drop-shadow-[0_0_15px_rgba(16,185,129,0.5)]">
          {t('pro') || (lang === 'az' ? 'LAYİHƏLƏR' : 'PROJECTS')}
        </h1>

        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8 md:gap-10">
          {projects.length === 0 ? (
            <p className="text-slate-500 col-span-full text-center text-lg">{lang === 'az' ? 'Hələ heç bir proyekt yüklənməyib.' : 'No projects uploaded yet.'}</p>
          ) : (
            projects.map(proj => {
              ...
              return (
                <Tilt key={proj.id} tiltMaxAngleX={5} tiltMaxAngleY={5} className="w-full h-full">
                  ...
                  >
                    ...
                    {/* VİTRİN QUTUSU (QAPAQ ŞƏKLİ) - İndi object-cover ilə tam dolacaq, bozluq qalmayacaq! */}
                    <div className="w-full aspect-[4/3] bg-[#02060d] overflow-hidden relative flex items-center justify-center border-b border-white/5">
                      <img 
                    ...
                    <div className="p-5 md:p-6 flex-1 flex flex-col relative bg-gradient-to-b from-transparent to-[#02060d]">
                       <h3 className="text-xl md:text-2xl font-black text-white mb-3 drop-shadow-md group-hover:text-emerald-400 transition-colors">
                         <AutoTranslate text={proj.title} lang={lang} />
                       </h3>
                       <p className="text-slate-400 text-xs md:text-sm mb-5 line-clamp-3 flex-1 leading-relaxed">
                         <AutoTranslate text={proj.description} lang={lang} />
                       </p>
                         ...
                         <div className="flex flex-wrap gap-2 mb-5">
                           {rawStats.map((stat, i) => (
                             <span key={i} className={`px-2.5 py-1 border text-[10px] font-black rounded-lg uppercase tracking-wider ${badgeColors[i % badgeColors.length]}`}>
                               <AutoTranslate text={stat} lang={lang} />
                             </span>
                           ))}
                         </div>
                       )}

                       {/* Texnologiyalar tərcümə edilmir (Orijinal qorunur) */}
                       {techList.length > 0 && (
                         <div className="pt-4 border-t border-white/10">
                           <p className="text-[9px] text-emerald-500 font-mono uppercase tracking-widest mb-2 font-bold">
                             {t('tech') || (lang === 'az' ? 'TEXNOLOGİYALAR' : 'TECHNOLOGIES')}
                           </p>
      ...
      {/* İRİ LAYİHƏ MODALI (SLIDER İLƏ) - Buradakı şəkillər kəsilmir (object-contain) */}
      <AnimatePresence>
        {viewProject && (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 z-[120] bg-black/95 backdrop-blur-xl flex items-center justify-center p-2 sm:p-4 md:p-8 pt-20 md:pt-4">
            ...
            <motion.div initial={{ scale: 0.95, y: 20 }} animate={{ scale: 1, y: 0 }} className="relative max-w-6xl w-full h-[85vh] md:h-[80vh] flex flex-col md:flex-row bg-[#050b14] border border-emerald-500/30 rounded-3xl md:rounded-[2rem] overflow-hidden z-10 shadow-[0_0_50px_rgba(16,185,129,0.15)]">
              ...
              <div className="flex flex-col md:flex-row w-full h-full overflow-hidden">
                ...
                {/* SOL: SLIDER HİSSƏSİ (Şəkil üçün tam sabit qutu, kəsilməyən) */}
                <div className="w-full h-[45%] md:w-[55%] md:h-full border-b md:border-b-0 md:border-r border-white/10 bg-[#02050a] relative flex items-center justify-center group overflow-hidden">
                ...
                <div className="w-full h-[55%] md:w-[45%] md:h-full p-6 md:p-10 flex flex-col overflow-y-auto custom-scrollbar bg-gradient-to-b from-[#0a1120] to-[#050b14]">
                  <h2 className="text-2xl md:text-4xl font-black text-transparent bg-clip-text bg-gradient-to-r from-white to-slate-400 mb-4 uppercase tracking-tighter pr-8 leading-tight shrink-0">
                    <AutoTranslate text={viewProject.title} lang={lang} />
                  </h2>
                  <p className="text-slate-300 text-sm md:text-base leading-relaxed mb-8">
                    <AutoTranslate text={viewProject.description} lang={lang} />
                  </p>
                  ...
                  <div className="flex flex-wrap gap-2 md:gap-3 mb-8 shrink-0">
                    {viewProject.speed && viewProject.speed.split(',').map((s: string, i: number) => (
                      <span key={`speed-${i}`} className="px-4 py-2 bg-emerald-500/10 border border-emerald-500/30 text-emerald-300 rounded-xl font-bold text-[10px] md:text-xs tracking-wide shadow-[0_0_15px_rgba(16,185,129,0.1)]">
                        <AutoTranslate text={s.trim()} lang={lang} />
                      </span>
                    ))}
                  </div>

                  <div className="mt-auto pt-6 border-t border-white/10 shrink-0">
                     <p className="text-[10px] text-emerald-500 font-mono uppercase tracking-widest mb-4 font-bold">
                       {t('tech') || (lang === 'az' ? 'İŞLƏDİLƏN TEXNOLOGİYALAR' : 'USED TECHNOLOGIES')}
                     </p>
