...
export default function Projects() {
  ...
  return (
    <main className="min-h-screen bg-transparent p-4 sm:p-6 md:p-10 pt-24 md:pt-32 transition-colors duration-500 relative z-10">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8 md:gap-10">
          {projects.length === 0 ? (
            ...
            projects.map(proj => {
              ...
              return (
                <Tilt key={proj.id} tiltMaxAngleX={8} tiltMaxAngleY={8} className="w-full">
                  <div className="bg-black/40 border border-white/10 rounded-[2rem] md:rounded-3xl overflow-hidden shadow-2xl group hover:border-emerald-500/50 transition-colors flex flex-col h-full cursor-pointer" onClick={() => setViewProject(proj)}>
                    ...
                    <div className="p-5 md:p-6 relative -mt-6 md:-mt-8 flex-1 flex flex-col">
                       <h3 className="text-xl md:text-2xl font-black text-white mb-2 md:mb-3 drop-shadow-md">{proj.title}</h3>
                       ...
                       {techList.length > 0 && (
                         <div className="pt-4 border-t border-white/10">
                           <p className="text-[9px] md:text-[10px] text-slate-500 font-mono uppercase tracking-widest mb-2">Texnologiyalar</p>
                           <div className="flex flex-wrap gap-1.5 md:gap-2">
                             {techList.map((tech: string, i: number) => (
                               <span key={i} className="px-2 py-1 bg-white/5 border border-white/10 text-slate-300 text-[10px] md:text-[11px] rounded-md shadow-sm">
                                 {tech}
                               </span>
                             ))}
                           </div>
      ...
      {/* İRİ LAYİHƏ MODALI (Mobil üçün zirehləndi) */}
      <AnimatePresence>
        {viewProject && (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 z-[120] bg-black/95 backdrop-blur-md flex items-center justify-center p-3 sm:p-4 md:p-8 pt-20 md:pt-4">
            ...
            <motion.div initial={{ scale: 0.95, y: 20 }} animate={{ scale: 1, y: 0 }} className="relative max-w-5xl w-full max-h-[85vh] md:max-h-[90vh] flex flex-col bg-[#0a1120] border border-white/10 rounded-3xl md:rounded-[2.5rem] overflow-hidden z-10 shadow-2xl">
              ...
              {/* Bağlama düyməsi mobildə şəklin üstündə görünməsi üçün arxa plan əlavə edildi */}
              ...
              <div className="flex flex-col md:flex-row h-full overflow-hidden">
                {/* Sol: İri Şəkil (Mobildə üstə, kompüterdə solda) */}
                <div className="w-full md:w-1/2 h-48 sm:h-64 md:h-auto border-b md:border-b-0 md:border-r border-white/5 bg-black/50 shrink-0 relative">
                  <img src={viewProject.image} className="w-full h-full object-cover md:object-contain md:p-4" alt={viewProject.title} />
                  <div className="absolute inset-0 bg-gradient-to-t from-[#0a1120] via-transparent to-transparent md:hidden"></div>
                </div>
                
                {/* Sağ: Məlumatlar (Sürüşdürülə bilən) */}
                <div className="w-full md:w-1/2 p-5 sm:p-6 md:p-10 flex flex-col overflow-y-auto custom-scrollbar">
                  <h2 className="text-2xl md:text-4xl font-black text-white mb-2 md:mb-4 uppercase tracking-tighter pr-8 md:pr-0">{viewProject.title}</h2>
                  ...
                  <div className="flex flex-wrap gap-2 md:gap-3 mb-6 md:mb-8">
                    {viewProject.speed && viewProject.speed.split(',').map((s: string, i: number) => (
                      <span key={`speed-${i}`} className="px-3 py-1.5 md:px-4 md:py-2 bg-cyan-500/10 border border-cyan-500/20 text-cyan-400 rounded-xl font-bold text-[10px] md:text-xs">
                        {s.trim()}
                      </span>
                    ))}
                    {viewProject.weight && viewProject.weight.split(',').map((w: string, i: number) => (
                      <span key={`weight-${i}`} className="px-3 py-1.5 md:px-4 md:py-2 bg-blue-500/10 border border-blue-500/20 text-blue-400 rounded-xl font-bold text-[10px] md:text-xs">
                        {w.trim()}
                      </span>
                    ))}
                  </div>
                  ...
                  <div className="mt-auto pt-5 md:pt-6 border-t border-white/5">
                     <p className="text-[9px] md:text-[10px] text-slate-500 font-mono uppercase tracking-widest mb-2 md:mb-3">İşlədilən Texnologiyalar</p>
                     <div className="flex flex-wrap gap-1.5 md:gap-2">
                       {viewProject.tech_stack && viewProject.tech_stack.split(',').map((t: string, i: number) => (
                         <span key={`tech-${i}`} className="px-2.5 py-1 md:px-3 md:py-1.5 bg-white/5 rounded-md text-[10px] md:text-xs text-slate-300 border border-white/10 font-medium tracking-wide">
                           {t.trim()}
                         </span>
                       ))}
                     </div>
